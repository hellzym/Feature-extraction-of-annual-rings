...
%);
linaccum_dr = [ (-rr_4linaccum(2) ) : -rr_4linaccum(1) , ...
    (rr_4linaccum(1) ) : rr_4linaccum(2) ];

lin2accum_aJ = floor( ...
	double(grdx(grdmasklin)./grdmag(grdmasklin)) * linaccum_dr + ...
	repmat( double(grdmask_IdxJ) , [1,length(linaccum_dr)] ) ...
);
lin2accum_aI = floor( ...
	double(grdy(grdmasklin)./grdmag(grdmasklin)) * linaccum_dr + ...
	repmat( double(grdmask_IdxI) , [1,length(linaccum_dr)] ) ...
);
